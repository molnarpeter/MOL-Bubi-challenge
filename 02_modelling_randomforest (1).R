...
rhs <- colnames(train)
rhs <- rhs[!(rhs %in% c("day","s_place_id","e_place_id","s_place_name",
                        "e_place_name", "usage","flag_train","tempm_avg_0",
...
newGrid = expand.grid(mtry = c(1,2,4,8,15))
for(i in c(1:length(top_routes_list))){
  print(paste("itt tart:", i))
  
  set.seed(42)
  model_rf <- train(as.formula(paste0("usage_pct~", varlist_to_rf)),
...
write.table(submission, 
            paste0(save_dir,"submission_23.csv"),
            sep = ',',
            row.names = FALSE,
            col.names = FALSE)
